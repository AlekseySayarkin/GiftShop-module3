...
public class PaginationValidatorImpl implements PaginationValidator {
    ...
    public void validatePagination(int page, int size) throws ServiceException {
        ...
        if (page <= 0) {
            throw new ServiceException(
                    "Failed to validate: page must be positive", ErrorCodeEnum.PAGINATION_VALIDATION_ERROR
            );
        }
    }
    ...
    public void validateSize(int size) throws ServiceException {
        if (size <= 0) {
            throw new ServiceException(
                    "Failed to validate: size must be positive", ErrorCodeEnum.PAGINATION_VALIDATION_ERROR
            );
        }
        if (size > MAX_AMOUNT_OF_ITEMS) {
            throw new ServiceException(
                    "Failed to validate: size is bigger then maximum allowed", ErrorCodeEnum.PAGINATION_VALIDATION_ERROR
            );
        }
    }
}
