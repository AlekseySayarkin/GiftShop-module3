...
public class UserValidatorImpl implements UserValidator {
    ...
    public void validateUser(User user) throws ServiceException {
        if (user == null) {
            throw new ServiceException("Failed to validate: user is empty", ErrorCodeEnum.USER_VALIDATION_ERROR);
        }
    ...
    public void validateId(int id) throws ServiceException {
        if (id < 0) {
            throw new ServiceException("Failed to validate: id is negative", ErrorCodeEnum.USER_VALIDATION_ERROR);
        }
    }
    ...
    public void validateLogin(String login) throws ServiceException {
        if (login == null || login.isEmpty()) {
            throw new ServiceException("Failed to validate: login is empty", ErrorCodeEnum.USER_VALIDATION_ERROR);
        }
    }
    ...
    public void validateUserSearchCriteria(UserSearchCriteria searchCriteria) throws ServiceException {
        if (!searchCriteria.getSortBy().equals(SortBy.LOGIN)) {
            throw new ServiceException(
                    "Cant sort users by " + searchCriteria.getSortBy(), ErrorCodeEnum.USER_VALIDATION_ERROR
            );
        }
    }
}
